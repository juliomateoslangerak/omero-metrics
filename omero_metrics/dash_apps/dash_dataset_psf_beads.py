...
def func_psf_callback(*args, **kwargs):
    ...
    bead_properties_df = kwargs[
        "session_state"
    ]["context"]["bead_properties_df"]
    df_properties_channel = bead_properties_df[
        bead_properties_df["channel_nr"]
        == channel_index
    ][
        [
            "bead_nr",
    ...
    fig_image_z.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=10,
                color=df_beads_location[
                    "considered_axial_edge"
                ].map(color_map),
                opacity=0.3,
            ),
            text=df_beads_location[
                "channel_nr"
            ],
            customdata=np.stack(
                (
            ...
            + "<b>Considered Axial Edge:</b> %{customdata[1]}\
             <br><extra></extra>",
        )
    )
    return (
...
def callback_mip(*args, **kwargs):
    ...
    if point["curveNumber"] == 1:
        ...
        title = f"MIP for bead number: {bead_index}\
...
def line_graph_axis(
    ...
    df_axis = kwargs["session_state"][
        "context"
    ][f"bead_{axis}_profiles_df"]
